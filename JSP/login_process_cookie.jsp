...
<%
	...
	String pass=request.getParameter("pass");//<input type="passoword" name="pass">
	
	//비밀번호가 암호화되어 저장되었기 때문에 로그인할때도 비밀번호를 암호화하여 비교 해야한다.
	pass=DataEncrypt.messageDigest("MD5", pass);
	
	//분리된 아이디와 비밀번호를 손쉽게 관리하기 위해서 VO에 설정한다.
	LoginVO lVO = new LoginVO(id, pass);
	
	//DAO단의 클래스를 수행하여 DB 작업을 수행한다.
...
<%		
		...
		if(idSaveFlag != null){ //value속성이 없는 상태에서 check하면 "on"이 들어온다.
			//2. 쿠키생성(파라미터로 바등ㄴ 아이디를 save_id라는 이름으로 쿠키 저장)
