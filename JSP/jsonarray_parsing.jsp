		...
		$.ajax({
			...
			success :function( jsonArr ){
				...
				$.each( jsonArr, function( i, jsonObj){
					//jQuery로 옵션을 동적으로 생성.
					//$("#selName:last").append("<option>"+ jsonObj.name+"</option>");
					output +=
				...
				selName.length=1;
				//3. 옵션을 생성. 옵션의 인덱스는 0부터 시작하기때문에 상황에 맞는 인덱스로 배열을 사용.
				var tempJson;
				for(var i=0 ; i < jsonArr.length ; i++){ //JSONArray를 for문을 사용하여 반복.
					tempJson=jsonArr[i];
					selName.options[ i +1 ]= new Option(tempJson.name, tempJson.name);
				}//end for
			}
		});//ajax
	});//click
